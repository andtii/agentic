...
- **Manifest** (#228, PLG-02): `learningDefaultPlugin` is the `PluginManifest` of `learningPlugin()` (same id and version) for the composition root's catalogue — kind `learning` (single-slot), `memory:read` + `memory:write`, no secrets; config `repeatThreshold` (integer 1–20, default 3: repetitions of one correction before an instruction change is proposed — the platform passes it to `learningPlugin`, #242).
