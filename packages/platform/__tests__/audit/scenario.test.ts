...
describe('the audit trail of a scripted scenario', () => {
    it('records each action kind exactly once per occurrence, lists with filters and a cursor, and survives a fresh host', async () => {
        ...
        // `chat.archived` is covered by the Chat archive tests (#774).
        ...
        // `plan.changed` / `plan.lease-expired` are covered by the Plan actor tests (#750); `request.changed` by the Requests actor tests (#758).
        ...
        const expected: Record<Exclude<AuditKind, 'workdir.worktree-created' | 'workdir.command-run' | 'project.chat-released' | 'connector.connected' | 'connector.disconnected' | 'connector.needs-reauth' | 'plugin.activated' | 'plugin.revoked' | 'plugin.tool-policy' | 'environment.put' | 'environment.removed' | 'chat.project-set' | 'chat.machine-set' | 'chat.archived' | 'project.changed' | 'session.interrupted' | 'session.resumed' | 'task.machine-lost' | 'machine.update-requested' | 'machine.updated' | 'machine.update-failed' | 'machine.channel-set' | 'machine.update-policy-set' | 'harness.changed' | 'machine.policy-set' | 'machine.renamed' | 'machine.restart-requested' | 'machine.restarted' | 'machine.login' | 'pull.merged' | 'pull.closed' | 'plan.changed' | 'plan.lease-expired' | 'request.changed'>, number> = {
