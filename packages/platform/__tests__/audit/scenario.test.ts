...
describe('the audit trail of a scripted scenario', () => {
    it('records each action kind exactly once per occurrence, lists with filters and a cursor, and survives a fresh host', async () => {
        ...
        // `chat.project-set` / `project.changed` are covered by the Chat and Workspace tests (#332).
        const expected: Record<Exclude<AuditKind, 'workdir.worktree-created' | 'plugin.activated' | 'environment.put' | 'environment.removed' | 'chat.project-set' | 'project.changed'>, number> = {
