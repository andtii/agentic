...
- Session files (#562): `fsRequest(environmentId, { kind: 'tree' | 'read' | 'changes', root, … })` is open to session drivers. A daemon without the `files` feature is answered `unsupported`, and a `root` outside the environment's `cwdRoots` is answered `outside-roots`, both at once with no frame sent. The answer is held by the activation, never saved, and read once from the `fsAnswer(requestId)` stream (`FsAnswer = WorkspaceAnswer<FsResult>`) or from `fsResult`. `changesSnapshot(environmentId, root, scope)` → `{ at, result } | null` is the last `changes` answer for that folder. At most `MAX_CHANGES_SNAPSHOTS` = 8 are kept, trimmed to `SNAPSHOT_MAX_FILES` / `SNAPSHOT_MAX_COMMITS`, and they survive disconnects. Read a folder through `machineWorkspaceSource(actor(Machine, key), environmentId, root, { base })`, a core `WorkspaceSource`; the Machine's 403 / 404 / 503 throw, and every other refusal is the answer's `error`.
