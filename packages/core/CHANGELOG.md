...
- `ChatEntry` status kinds `request` / `request-resolved` (#40): a session's open request and its settlement as chat status entries; `ref` is `approval:{requestId}` or `input:{requestId}` on both so they pair up.
