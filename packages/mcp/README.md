...
Both take an optional `tools: McpConnectorTool[]` — the tools a probe of the server found, each `{ name, title?, description?, annotations? }` as `tools/list` gave it (name unprefixed). They become `manifest.tools` under their session names (`connectorToolPrefix(id)` + the tool name, via `toolNameFor`: `git.hub` + `delete.repo` → `git_hub__delete_repo`), with `title` falling back to `annotations.title`, a `defaultMode` from `defaultToolMode(annotations)` (`ask` for a `destructiveHint` tool not marked `readOnlyHint`, else `allow`), and a name seen twice declared once. Omitted: the manifest declares no tools; `tools: []` declares an empty list. The stored `connector` record never carries them.
...
`openMcpConnector({ id, url, bearer?, headers?, fetch?, allowedHosts?, timeoutMs? })` → `{ tools, toolNames, close }` connects, lists and namespaces every tool `<id>__<tool>` (`connectorToolPrefix(id)`; an id with a `.` becomes `_`), with one deadline (default 10 s) for the handshake and the list. The platform injects it as the `connectors` opener of its local runtime (`apps/web/src/plugins/catalogue.ts`); credential values arrive already opened and go only into request headers. `allowedHosts` (the connector plugin's granted `network:` hosts, #642; also on `createMcpClient`) puts the fetch behind `guardFetch`: a request to any other host fails with `McpNetworkError` naming the scope, before it is sent. `guardFetch` follows redirects itself (`redirect: 'manual'`), checking every hop, so a granted server cannot redirect the client to a host that is not granted. On the daemon (#280), `@agentic/mcp/node` adds `openStdioMcpConnector({ id, command, args?, cwd?, env?, timeoutMs? })`: the same shape for a stdio server. It spawns the server with the allowlisted environment plus the credential variables, gives spawn + handshake + list one deadline, and kills the server on close, including one that came up after the deadline. The daemon's `openConnector` picks one of the two by transport.
...
- **Tools** (`platformTools(port, principal)`), named `<family>_<op>` because provider tool names cannot carry a dot; the family is the OAuth scope that gates it — a call without it is an `isError` result naming the scope, never a silent no-op:
  ...
  | `sessions` | `sessions_open(agentId, machineId, environmentId, cwd?, objective?)`, `sessions_prompt(sessionId, text)`, `sessions_respond(sessionId, requestId, decision)`, `sessions_cancel(sessionId)`, `sessions_tail(sessionId, from?, limit?)`, `sessions_files_list(sessionId, path?)`, `sessions_files_read(sessionId, path, rev?)`, `sessions_changes(sessionId, scope?)` (#566: the session's folder, read-only) | open/prompt/respond write, cancel destructive, tail and files readOnly |
...
- **Session files** (`sessions_files_list`, `sessions_files_read`, `sessions_changes`, #566): the folder a daemon-hosted session runs in, read-only — the same data as the web's Changes and Files views. They call `PlatformPort.sessions.tree` / `read` / `changes`, which answer a `WorkspaceAnswer` (`@agentic/core`): the result, or the machine's refusal. The tools return a refusal as an error result, `<tool>: <code>: <message>`. Paths are relative to the session folder and `/`-separated; `sessions_files_list` without `path` lists the folder itself; at most 500 entries, and ignored files are left out when `ignoredHidden`. `sessions_files_read` takes `rev`: `working` (default), `head` or `base`. Compare two revs to see what the agent changed. Like `chats_file_get`, `content[0]` is a JSON summary `{ sessionId, path, rev, uri, kind, size, lines?, note? }` (the `structuredContent`; `uri` is the `agentic-session://` URI a chat message cites). A text file's text follows in `content[1]`; a binary file is the summary alone; a text file over the machine's read limit (480 KB) is `too-large`. `sessions_changes` takes `scope`: `uncommitted` (default) or `branch`. It answers a VCS-neutral `ChangeSet`: files with a status and line counts, the branch's commits, and `branch` / `base` / `ahead` / `behind`. A folder without version control is `not-a-repo`. The app's port reads the Session as the client, so the `sessions` scope is all a client needs, and refuses a session that has no folder on a machine (an API runtime).
