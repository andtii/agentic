...
- `MarkdownViewer value` is a document as prose: the `ag-markdown` recipe styles what `@sigx/markdown/dom` renders inside it (`@sigx/markdown` ships no stylesheet) — headings, lists and task boxes, blockquotes, code blocks with a language / copy header, tables, links (`_blank`, or `onLink`), images. Code highlights through one shared shiki highlighter (`markdownHighlighter()`, loaded on the first block, plain text on any failure); `highlighter={false}` keeps it plain (tests). `compact` is the card-well size.
- `MarkdownDialog model title value` reads a document full-size: zero's `Dialog` at 880 px, the viewer scrolling between the title and the footer, the `footer` slot's actions before `Close`; a close no action preceded emits `close`. The plan card opens its plan in it.
