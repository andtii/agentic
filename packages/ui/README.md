...
zero-daisyui's tokens and recipes, re-tuned to the handoff and compiled by `sigx zero:build` (part of `build`) into `dist/ds/**`. One theme, `control-room`, dark only. The handoff's extras are `--ag-*` custom tokens (`--ag-line`, `--ag-text-dim`, `--ag-agent-1`..., `--ag-control-h`, `--ag-sidebar-w`, ...); product states ride the `tone` and `kind` axes and the `hollow` / `outline` / `compact` / `selected` / `current` modifiers, not `data-state`.
...
The app components of `docs/design/HANDOFF.md` → "Components", one visual per domain state. The `ag-*` scopes ship in the fragment with recipes (`kitAnatomies` is the list; among them `WaitReasonLine`, `EnvironmentLine`, `NeedsItem`, `TaskNode`, `ConnectionStrip`, `VersionItem`, `EnvironmentCard` on `ag-env-card`, `MarkdownViewer` on `ag-markdown`); the rest compose zero, their look in the design system's patch for the zero scope (`Button`, `StatusPill` / `Tag` on `Badge`, `AgentTile` on `Avatar`, `Segmented` and `FilterChips` on `ToggleGroup`, `Switch`, `ChipInput`, `DataTable`, `TimelineList` on `Timeline`, `ConfirmDialog`, `FormDialog`, `MarkdownDialog`, `ErrorNote`, `SectionHeading`, `Label`, `Icon`). Product state never rides `data-state`: a colour is the `tone` axis (`data-tone`) or, on a zero part, the tone's role (`roleOf`: live → primary, working → info, needs-you → warning, failed → error, muted / dim → neutral) with `data-tone` kept on the root; an inbox row's kind is the `kind` axis, presence flags are `data-mod-*`.

- `StatusPill status label tone hollow` is zero's `Badge.Root` in the tone's role, `soft`, or `outline` when hollow (the dot a ring), with `Badge.Dot` — `running` while the state is working (ACTIVE, RUNNING, STREAMING). `data-status` and `data-tone` stay on the root. `Tag tone` is the same badge, `outline`, no dot, `data-tag` on the root.
- `AgentTile name hue size person labelled` is zero's `Avatar.Root` with the `shape` axis (`square` for an agent, `circle` for a person) and the monogram as `Avatar.Fallback`. `data-hue` and `data-tile` (the size) are the hooks the avatar patch turns into `--ag-hue` and the 18–52 px `--ag-tile`. `labelled` makes it `role="img"` named by `name`; otherwise it is `aria-hidden` with the name as its `title`.
- `TimelineList entries label` colours each `Timeline.Marker` by its entry's tone (`color={roleOf(tone)}`, `data-tone` kept) and names the list with `label`; the time's mono voice is the timeline patch's.
- `FilterChips options label model name` is the filter bar (`Filter by status`, `Filter by kind`): zero's `ToggleGroup`, single value, not deselectable, arrow keys between chips, each chip's optional `count` after its label (`[data-chip-count]`), `name` to post it inside a form. The chip look — 22 px mono pill, pressed = the primary at 15 % — is the toggle-group patch's, keyed on the root's `data-filter-chips`.
...
- `CategoryMenu label groups current` is a catalogue's category column (board `Plugins`, 232 px): a `<nav aria-label>` on zero's `NavList`, each group under a mono label, 34 px links with a mono count and `aria-current="page"` on the item whose `id` is `current`. A count of `0` shows in `text-dim` and the link stays clickable; `badge` paints a non-zero count as the amber `needs-you` badge.
- `SearchField model label placeholder` is a page search: a `role="search"` landmark over zero's `Input` (`type="search"`) with a visually hidden `<label>`, a search icon and a `/` hint chip (zero's `Kbd`). `/` anywhere on the page focuses it (`preventDefault`), except while typing in an input, textarea, select or contenteditable (`isTypingTarget`); the document listener goes with the component.
...
- **`QuotaMeter`** (`ag-quota`) draws one `QuotaWindow`:
  - the bold label
  - an 8 px bar coloured by status (`ok` → `live`, `warning` → `needs-you`, `exhausted` → `failed`, `unknown` → `muted`): zero's `Progress.Root` (`data-quota-bar`) in the tone's role, named by the window with "76% used" as its `aria-valuetext`; a window without a number is indeterminate, drawn empty
  - "76% used"
...
- **`LineComposer`** (`ag-line-composer`): "Ask <agent> about line n", `file:line`, zero's `Textarea` autosizing from two rows (Ctrl/Cmd+Enter sends, Escape cancels), the "Posts to …" note, Cancel and Send. It only collects the question; posting is the caller's.
...
The handoff's failure, empty and loading states, one component each, so no page invents "Something went wrong" (OPS-04, OPS-05). `FailureCard kind=…` renders one of the six named states on zero's `EmptyState` — `client-offline`, `machine`, `auth`, `runtime`, `task`, `interrupted` — each with its own name, icon, mono signal caption, tone and action, all from the `FAILURES` table (`kit/states/kinds.ts`, shared with the inbox row's kinds): `color` is the tone's role, the `kind` axis (`data-kind`) the design system's spelling (`client-offline` → `offline`), the root `role="status"` named by the state with `data-failure` and `data-tone`, and the title row `[data-failure-icon]`, `[data-failure-name]`, `[data-failure-signal]`. `OfflineBanner` (`ag-banner`) is the "This browser is offline · Reconnecting…" strip over the content; `EventsLostRow` the amber "Events lost between seq A and B" line in a session log; `browserRow` / `machineRow` / `connectionRows` turn the two signals into `ConnectionStrip` rows (hollow dot when nothing is happening). `EmptyState variant=…` (zero's `EmptyState`, `role="status"`, the variant as `data-empty`, `compact` / `outline` as `data-mod-*` the empty-state patch draws, a default action as the kit `Button` link) is the workspace card, the inbox line, the dashed machines card, the chat hint, or a generic title + caption. `TableSkeleton` / `CardSkeleton` / `RailSkeleton` stand in while data loads, one `aria-busy` region with a hidden "Loading" each. Mapping live signals to these is #46's job.
...
- `ToolCall` paints the lifecycle on zero's governed lifecycle states — `loading` (pending, awaiting approval), `running`, `complete`, `error` (failed), `denied`, `cancelled`, and a sub-agent's `paused` — with the handoff's word on a `StatusPill` in `status` (PENDING · RUNNING · DONE · ERROR · DENIED) and the refined phase (`awaiting approval`, `cancelled`, `done, no output`) as the header `meta` when the caller gave none. The border takes a colour only while `running` and on `error`. The input, the output well and a sub-agent's work fold on zero's `Collapsible`; a reader's toggle survives streaming updates. The output well shows six lines, then "Show N more lines" (`more`); past 200 lines the rest is in the session log (`log`, needs `logHref`).
- `Reasoning` folds to "Reasoning · Ns" behind a chevron once done (`seconds`), open while it streams; the reader's toggle wins. It is zero's `Collapsible` (a native `<details>`) inside the `ai-reasoning` root, its `model` following the part until the reader toggles it.
...
- `Composer` emits `send(text)`, `cancel` and `attach`; pass `busy` / `steers` / `canCancel` from the agent's capabilities, `mentions` for the `@` popup, and `recipients` + `hint` for the "To" row (an empty list says nobody will answer — the page resolves who, per CHT-06). The popup is zero's `Combobox trigger="@"` around the textarea, anchored at the caret: prefix matches first, at most `MAX_MENTIONS` (`filterMentions`); with it open, Enter or Tab picks and never sends. Otherwise Enter sends, Shift+Enter breaks a line, and an IME composition's Enter is the composition's. The box autosizes between `minRows` and `maxRows` (1–8).
