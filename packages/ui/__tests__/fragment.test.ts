...
describe('the ai-* fragment', () => {
    ...
    it('owns the seven transcript scopes of the architecture and the kit ag-* scopes, vendor-prefixed', () => {
        ...
        expect(SCOPES.slice(7)).toEqual(['ag-pill', 'ag-agent-tile', 'ag-env-line', 'ag-needs-item', 'ag-task-node', 'ag-connection', 'ag-version', 'ag-env-card', 'ag-failure', 'ag-banner', 'ag-empty', 'ag-workdir', 'ag-workdir-picker', 'ag-plugin-card', 'ag-secret', 'ag-map-field']);
