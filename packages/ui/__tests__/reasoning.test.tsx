...
import { Reasoning, aiReasoningAnatomy } from '../src/thread';
...
describe('reasoning', () => {
    ...
    it('shows exposed reasoning open while streaming and folded once done', () => {
        ...
        const done = mount(<Reasoning part={{ type: 'reasoning', id: 'r1', text: 'weighing INC-41', done: true }} />);
        expect(one(done, 'ai-reasoning', 'root')!.getAttribute('data-state')).toBe('closed');
        expect(one(done, 'ai-reasoning', 'summary')!.textContent).toBe('Thought');
