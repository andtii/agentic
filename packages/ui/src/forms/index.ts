/**
 * Forms (issue #25): agent configuration, the environment card, plugin
 * settings from a schema and the working-folder picker (#191), all on
 * zero's `model=` form contract — a `Field` around every control, a real `name` on every control so the form posts pre-hydration,
...
export { WORKDIR_EMPTY, FS_ERROR_TEXT, workdirLabel, workdirPath, workdirCrumbs, middleTruncate, samePath, fsErrorText, toWorkdirEnvironment } from './workdir-model.js';
