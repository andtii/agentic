...
export type QuotaMeterProps =
    ...
    /** The reset line without the zone, `Resets Thu 25 Sep 12:00` — the zone is said nearby (#452). */
    & Define.Prop<'shortResets', boolean>
    & Define.Prop<'class', string>;

export const QuotaMeter = component<QuotaMeterProps>(({ props }) => () => {
    const w = props.window;
    const percent = quotaPercent(w);
    const at = { ...(props.now !== undefined ? { now: props.now } : {}), ...(props.timeZone ? { timeZone: props.timeZone } : {}) };
    const resets = props.shortResets ? resetsShortText(w.resetsAt, at) : resetsText(w.resetsAt, at);
    return (
...
export type QuotaBadgeProps =
    ...
    /** The model the member runs (#452): another model's window is not its limit; without one, only the shared windows count. */
...
/**
 * One account's limits in one line (#315): of the windows that limit `model` (`memberWindows`, #452), the one closest to its limit as a compact meter, or a muted note —
...
export const QuotaBadge = component<QuotaBadgeProps>(({ props }) => () => {
    ...
    const mine = s ? memberWindows(s, props.model) : [];
    const w = tightestWindow({ windows: mine }) ?? mine[0];
...
export type QuotaPanelProps =
    ...
    & Define.Prop<'staleMs', number>
    /** Say the time zone once, at the header's end, and each window's reset in the short form (#452). */
    & Define.Prop<'zoneInHeader', boolean>
    & Define.Prop<'class', string>;
        ...
        <section data-scope={PANEL} data-part="root" data-mod-stale={stale ? '' : undefined} data-mod-compact={props.compact ? '' : undefined} data-availability={s?.availability ?? 'none'} aria-label={props.title ? `Usage limits: ${props.title}` : 'Usage limits'} class={props.class}>
            {props.title || s ? (
                <div data-scope={PANEL} data-part="header">
                    ...
                    {props.zoneInHeader && windows.some((w) => w.resetsAt) ? <span data-scope={PANEL} data-part="zone">{props.timeZone ?? Intl.DateTimeFormat().resolvedOptions().timeZone}</span> : null}
                </div>
            ) : null}
            {reason ? <p data-scope={PANEL} data-part="reason">{reason}</p> : null}
            {windows.map((w) => <QuotaMeter window={w} stale={stale} now={now} shortResets={props.zoneInHeader} {...(props.timeZone ? { timeZone: props.timeZone } : {})} />)}
        </section>
    );
}, { name: 'QuotaPanel' });
