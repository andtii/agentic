...
The machine-managed-from-the-web frames (#355; features `policy`, `log`, `login`): `policy.request { op: 'set', policy: { allowedRoots } }` is strict and names at most `LIMITS.policyRoots` (32) folders of at most `LIMITS.policyRoot` (1 KiB) each — `~` forms included, which the machine expands — and a reported policy's `allowedRoots` / `requested` are bounded the same; and `op: 'browse', path?` lists one folder's subfolders (or the machine's roots); `policy.response` is exactly one of `result: { policy }` (the policy as applied: `source: 'web'`, `requested` echoing the input, `locked` when the owner locked it on the machine), `result: { listing }` (≤ `FS_LIST_MAX_ENTRIES`, folders only) or a named `error` (`policy-locked`, `invalid`, `not-found`, `not-a-directory`, `remote-path`, `protected`, `io`, `unsupported`; `timeout` is the platform's). `log.request { lines }` asks for 1..`LIMITS.logLines` (500) and `log.response` carries at most that many, or `no-log` / `io` / `unsupported` (`timeout` is the platform's, like the policy's). `login.request { environmentId }` starts a relayed sign-in; `login.status` passes `started` → `action` (with the `action` exactly then: `open-url` or `device-code`, `expectsPaste`) → `waiting` → `done` | `failed` (with `error` exactly then: `busy`, `unknown-environment`, `unsupported`, `cancelled`, `timeout`, `failed`); `login.answer { text }` is at most `LIMITS.loginAnswer` (2 KiB) and nothing else; `login.cancel` ends one. A `CapabilityReport` may say `login: 'relay' | 'terminal'` (#484) — whether the daemon relays that runtime's sign-in or only `env login` on the machine does; absent from an older daemon (read as `terminal`). `env.request { op: 'put' }` may now carry `allowBypassPermissions`; `profileDir` is still refused.
...
| `files-tree` | `hello.features` lists `files`; `fs.request` `tree` of the harness's `files.root` lists the first folder of `files.file` as a `dir` and never `.git`; a `path` climbing out of `root` and a `root` outside the working roots answer `outside-roots` (OPS-01), an unknown environment `unknown-environment` — feature `files` (#559) |
| `files-read` | `read` of `files.file` answers exactly its text at `rev: 'working'`; a path outside `root` answers `outside-roots`, a missing file `not-found` — feature `files` |
| `files-changes` | with `files.changed`, `changes` `uncommitted` names its VCS and scope and lists `files.file`, and its `head` text differs; `files.plain` answers `not-a-repo` — feature `files` |
...
A harness implements `DaemonConformanceHarness`: `start(script)` returns a `ConformanceDaemon` (`machineId`, `environmentId`, `dial()` → a `PlatformSeat` with `send` / `next` / `drop`, `stop()`, and the optional `setEnvironments` / `truncateLog` / `setPolicy` / `lock` / `restart` behind `features`); an optional `files` (`ConformanceFiles`: a `root` holding `file`, whether it is `changed`, a `plain` non-repo folder) is what the `files` cases read (#559), an optional `knownOrigin` names a remote URL with a checkout under the suite environment's roots so `fs-locate` proves a match, `updateTarget` / `harnessTarget` name what `update` / `harness` install, and for #355 `protectedFolder` (a folder of the daemon's own that `policy` must refuse and never list), `logLines` (how many lines the `log` holds) and `loginAction` / `loginAnswer` (what the `login` relay will show, and the paste that completes it). `inMemoryHarness()` is the reference fake — also a stand-in daemon for tests of the platform side: it answers `history.request` from the per-session log it keeps, reports `IN_MEMORY_BUILD`, fakes an update (phases a tick apart, a drain that waits for running turns) and harness installs without downloading anything, and `restart()`s keeping its logs so a `wanted` session is answered `restart` and re-opened from `spec.resume` (#363) — its `repos` option fakes git checkouts (badged in listings, found by `locate`; the first with an `origin` becomes `knownOrigin`), its `folders` option the session folders `tree` / `read` / `changes` answer from (#559; default `IN_MEMORY_SESSION_FOLDERS`, a repository at `/work/project` and a plain `/work/plain`, read by the `files` cases through `IN_MEMORY_CONFORMANCE_FILES` — custom `folders` need `conformanceFiles` to keep the feature on), its `log` and `login` options script the log it tails and the sign-in it relays (#355; it takes a web policy with `~` expanded to `IN_MEMORY_HOME`, refuses `IN_MEMORY_OWN_DIR`, and `lock()`s), and its `faults` option breaks it on purpose so a test can check the suite notices.
