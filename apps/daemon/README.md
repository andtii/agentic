...
| harnesses `<runtime>/<version>/`, `<runtime>/current.json` | `%LOCALAPPDATA%\agentic\harnesses` | `~/.agentic/harnesses` | same |

`AGENTIC_DAEMON_HOME` puts all of them but the harnesses in one directory; the harnesses follow the install root (`AGENTIC_INSTALL_DIR`).
