...
export function createDaemon(options: DaemonOptions): Daemon {
    ...
    const features: DaemonFeature[] = ['files', 'run', 'worktrees', 'pin', ...(updater ? (['update'] as const) : []), ...(options.harnesses ? (['harness'] as const) : []), ...(options.webPolicy ? (['policy'] as const) : []), ...(options.logTail ? (['log'] as const) : []), ...(options.login ? (['login'] as const) : [])];
