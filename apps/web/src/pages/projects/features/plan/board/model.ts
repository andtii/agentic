 ...
 */
import { memberLimit, planItemWaitsOn, planTouchesOverlap, type AgentId, type PlanActor, type PlanItem, type ProjectMembers } from '@agentic/core';
...
/** Whether the item is being worked now: a claimed (or stuck / needs-you) item with a claim on it. */
export const isWorking = (item: Pick<PlanItem, 'claim' | 'state'>): boolean => item.claim !== undefined && item.state !== 'done' && item.state !== 'ready';
 ...
 */
export function boardColumns(items: readonly PlanItem[], members: ProjectMembers): BoardColumn[] {
    ...
    const column = (key: BoardColumnKey, kind: BoardColumn['kind'], agentId?: string): BoardColumn => {
        ...
        const working = kind === 'agent' ? mine.filter(isWorking).sort((a, b) => a.id - b.id) : [];
...
export function needsHandoff(item: PlanItem, slot: BoardSlot): boolean {
    return isWorking(item) && columnOf(item) !== slot.column;
}
...
export function isNoop(item: PlanItem, slot: BoardSlot, columns: readonly BoardColumn[]): boolean {
 ...
 * a `note` goes into its history as the handoff.
 */
export function moveItem(items: readonly PlanItem[], id: number, slot: BoardSlot, by: { readonly you: PlanActor; readonly at: number; readonly note?: string }): PlanItem[] {
    ...
    const queueOf = (key: BoardColumnKey): PlanItem[] => items.filter((i) => i.state !== 'done' && columnOf(i) === key && !(key !== 'open' && key !== 'you' && isWorking(i))).sort(byQueue);
    ...
    const handoff = needsHandoff(item, slot);
...
export function cardMeta(item: PlanItem, items: readonly PlanItem[], now: number, nameOf: (actor: PlanActor) => string): string[] {
    ...
    if (isWorking(item)) {
        const clash = items.filter((o) => o.id !== item.id && isWorking(o) && o.id < item.id && o.touches.some((t) => item.touches.some((u) => planTouchesOverlap(t, u))));
        if (clash.length) meta.push(`overlaps ${clash.map((o) => `#${o.id}`).join(', ')} · waits to rebase`);
    }
    const waits = planItemWaitsOn(item, items);
    if (waits.length) meta.push(`${isWorking(item) ? 'waits on' : item.queueIndex !== undefined && item.assignee ? 'after' : 'waits on'} ${waits.map((n) => `#${n}`).join(', ')}`);
