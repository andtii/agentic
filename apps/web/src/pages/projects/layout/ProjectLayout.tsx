...
const frame = (p: ProjectRecord, Inner: ProjectPage) => (
...
const MockProjectLayout = component<LayoutProps>(({ props }) => () => {
    ...
    return p ? frame(p, props.page) : notFound(props.id);
...
const LiveProjectLayout = component<LayoutProps>(({ props }) => {
    ...
    // The head this instance published: a later layout (the next project page) may already have replaced it.
    ...
    return () => {
        ...
        const p = projects.byId(props.id);
        if (p) return frame(p, props.page);
