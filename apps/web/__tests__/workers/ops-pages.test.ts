...
describe('worker: the ops pages over the browser stubs', () => {
    it('a reminder at a wall time on the workspace zone arms the Schedule object for that instant; the Registry lists secrets by name', async () => {
        ...
        expect((await registry.list()).map((p) => p.manifest.id)).toEqual(['agentic.a2a.server', 'agentic.learning.default', 'agentic.memory.default', 'agentic.memory.flat', 'agentic.notify.web-push', 'anthropic-api', 'claude-code', 'codex-cli', 'copilot-cli']);
