...
import { AgentActor, AuditActor, Chat, ChatPage, LedgerActor, Memory, PairingDirectory, TaskActor, TaskIndex, Workspace, agentKey, createToolCallPort, defineInbox, defineMachineActor, defineRegistry, defineRoutingActor, defineSessionActor, sessionPolicy, workspaceKey, type MachineSocketPort, type SessionFactory } from '@agentic/platform';
...
export async function startLive(agentScript: MockAgentOptions = { respond: (input) => [{ text: `echo: ${input.map((p) => (p.type === 'text' ? p.text : '')).join('')}` }] }, options: LiveOptions = {}): Promise<LiveHarness> {
    ...
    const base = [Workspace, AgentActor, Chat, ChatPage, TaskActor, TaskIndex, Session, Routing, Machine, PairingDirectory, Inbox, Memory, LedgerActor, AuditActor, defineRegistry()].filter((d) => !extra.some((e) => typeOf(e) === typeOf(d)));
