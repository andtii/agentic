...
describe('the machine setup model', () => {
    ...
    it('files the environment audit kinds under Machines, linked to the machine and toned by outcome', () => {
        expect(HISTORY_KIND_FILTERS.find((f) => f.id === 'machines')!.kinds).toEqual(['machine.paired', 'machine.revoked', 'environment.put', 'environment.removed', 'machine.update-requested', 'machine.updated', 'machine.update-failed', 'machine.channel-set', 'machine.update-policy-set', 'harness.changed']);
