...
describe('AC-13: a plugin is disabled', () => {
    it('names its dependents, refuses new use with a typed error, and leaves running references as they are', async () => {
        ...
        // Web Push (#244) and the A2A server (#245) ship turned off (the flat memory plugin is on now that it is durable, #281); every other built-in is on — and all stay as they were.
        expect(await builtins()).toEqual(builtinsBefore);
        expect(builtinsBefore.filter(([, on]) => !on).map(([id]) => id)).toEqual(['agentic.a2a.server', 'agentic.notify.web-push']);
