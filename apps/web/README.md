...
- `src/styles.css` (document surface) and `src/styles/pages.css` (per-screen grids: an ordered `@import` of `styles/pages/<area>.css`, one file per page area, media queries on the design system's `@custom-media`).
