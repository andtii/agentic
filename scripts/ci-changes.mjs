 ...
 * - With a ref (locally): diffs `<ref>...HEAD` plus the working tree.
 */
...
if (process.env.ALL === '1') {
    ...
    const files = base ? [...git('diff', '--name-only', `${base}...HEAD`), ...git('diff', '--name-only', 'HEAD')] : git('diff', '--name-only', 'HEAD^1', 'HEAD');
