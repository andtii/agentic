...
| Plugins beyond the runtimes | the build's plugins are listed and configured at `/plugins` and the runtimes are load-bearing (#229 – #231, #233: `Routing.run` gates on them, the `anthropic-api` key is the workspace's Registry secret); memory, learning and notification channels are still wired statically and a stored MCP connector is never read by a session. The A2A server is mounted as a plugin, off by default (#245, §9 "Expose agents over A2A"), with live A2A task state per isolate; remote A2A agents as runtimes are not wired | #240 – #244, #246 (decisions 2026-09-19) |
