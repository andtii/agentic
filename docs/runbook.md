...
| Entering the workspace's Anthropic key in the UI | the key is the workspace's `anthropic-api-key` Registry secret and the deployment holds none (#231); until the `/plugins/anthropic-api` page lands it is set over the actor mount (`Registry.setSecret`, as the demo 1 smoke does) | #233 |
