...
| Plugins beyond the runtimes | the build's plugins are listed and configured at `/plugins` and the runtimes are load-bearing (#229 – #231, #233: `Routing.run` gates on them, the `anthropic-api` key is the workspace's Registry secret); memory and learning follow the workspace's active plugin and its config (#242: turned off, a session retrieves, writes and learns nothing). Switching the active memory plugin moves no memories yet (#243), and the flat memory plugin ships turned off: this build keeps its entries in the isolate's memory, so they are lost when the isolate goes, and it serves an agent's own scope only (#281). The A2A server is mounted as a plugin, off by default (#245, §9 "Expose agents over A2A"), with live A2A task state per isolate; remote A2A agents as runtimes are not wired. Notification channels are still wired statically and a stored MCP connector is never read by a session | #240, #241, #243, #244, #246, #281 (decisions 2026-09-19) |
