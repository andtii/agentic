...
| Adding a working root from the web | the folder picker (#193) browses and creates worktrees only inside an environment's `cwdRoots`; a new root is added in the machine's `environments.json` and takes a daemon restart (it reads the file once) | #235, then #236 – #239 (decisions 2026-09-19 (c)) |
