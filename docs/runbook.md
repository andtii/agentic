...
Pairing codes come from `Workspace.registerMachinePending({ name, allowedRoots })`: six characters, single use, valid 10 minutes; the daemon presents it to `POST /auth/pair` and receives the machine token.

Signed in, open **Machines → Pair a machine** (`/pair`, #144). The page mints a code the moment it opens, under the name in its **Machine name** field (`machine-N` by default) and the **Folders the web may use** below it (`~`, the daemon user's home folder, by default; one per line — change either and a fresh code is minted, so the platform's record and the daemon agree). The folders ride the pending record and become the machine's policy on its first hello (#482, #480), so a fresh machine is usable from its page at once — no command typed on it after the install line. The page shows, with the code, this origin and the name filled in:

- step 1, one install line per OS (§5.3);
- step 2, the by-hand pair command: `agentic-daemon pair <code> --url <origin> --name <name>` (full paths from the folders field ride it as `--allow-root`, for a headless install; a `~` form is the platform's to expand).
...
|---|---|---|
| `AGENTIC_CHANNEL` | `stable` (since `daemon-v0.1.1`; §5.1) | the newest build of `latest` (`daemon-latest`, every `main` push) or `stable` (`daemon-stable`, the newest `daemon-v<semver>` tag) |
...
**Everything else is on `/machines/:id`** (#355; #478 – #484). The install line is the only thing typed on the machine. The page's setup checklist (Paired → Folders → Environment → Signed in → Ready) names the next step and opens it:

- **Folders the web may use** — the machine's policy, set from the page (`Machine.setPolicy`): each root as asked beside what the daemon made of it (`~ → C:\Users\me`), add by path or **Browse…** the machine, **Save folders**. The Pair page's folders arrive here as the first set. Saving asks you to confirm with GitHub once (a ten-minute elevation; §9 "elevation-required"); the daemon still refuses network shares, non-folders and its own folders, whoever asks; `agentic-daemon policy lock` on the machine always wins (§9 "the card says locked").
- **Environments** — **Add environment** / Edit / Remove inside the allowed folders (`docs/multi-account.md`): a runtime, its account label, the working folders, concurrency, and the **Allow bypassPermissions** switch (Claude Code runs every tool unasked in that environment; turning it on is the same GitHub confirmation; without it the mode is not offered and a task asking for it fails `permission-mode-not-allowed`, #453). The daemon allocates the profile folder itself; nothing on the wire names one.
- **Sign in…** on an environment's row (#484) — the runtime's own login relayed from the machine: a link to open in any browser (Claude Code then wants the code pasted back — it reaches the runtime once and is kept nowhere) or a device code to enter at a URL (Codex, Copilot). The row flips to the account when the daemon re-inspects it. A runtime whose CLI is not on the machine (Copilot without `copilot` on `PATH`) keeps the copyable `agentic-daemon env login <id>`.
- **This machine** — Rename, **Restart…** (drain or now; §5.4), the **Daemon log** (the last 200 lines, token-redacted), Remove from workspace. **Daemon updates** and **Runtimes on this machine** (§5.5, §5.6) as before.

Every change is audited under the user who made it (History → Machines), the folders and bypass changes also land in the Inbox (`machine-security`) and in the daemon's log, and none of the methods is on any tool surface — no agent, MCP client or external principal can widen where agents work (decisions 2026-09-22).

**By hand, or headless.** The same things from the machine's own terminal, for a box nobody browses from or a daemon that predates the feature (`hello.features` without `policy` / `log` / `login`; the page then shows the command): `agentic-daemon env add --name Work --root <folder> [--concurrency 2] [--account me@work.example] [--allow-bypass]` (before #354, or from an unpacked zip: `node bin/agentic-daemon.mjs …`) writes `environments.json` atomically, owner-only, and gives the environment its own profile folder; `env list` and `env rm <id>` do the rest; `agentic-daemon env login <id>` signs one in (`claude /login` under that profile with the installed Claude Code harness, no `claude` on `PATH` needed; Codex likewise; Copilot with `copilot login` from `PATH`); `agentic-daemon policy allow-root <folder>` / `deny-root` / `off` set the folders locally (`source: local`), `policy lock` / `unlock` keep the web from changing them, `policy show` prints them with their source. A daemon with no environments connects and reports none, a running daemon watches `environments.json` and `policy.json` and announces a change within a second, and environments that are not signed in are re-checked every 30 s — none of it takes a restart. A hand edit that does not validate is logged (`environments.json is invalid; keeping the running environments`) and ignored.
...
|---|---|---|---|
| `credentials.json` (machine token, owner-only), `environments.json`, `policy.json` (`{ webManaged, allowedRoots, source: 'local' \| 'web', requested?, locked? }` — `requested` the roots as the web asked, `~` unexpanded; `locked` from `agentic-daemon policy lock`), `profiles/` | `%APPDATA%\agentic` | `~/Library/Application Support/agentic` | `~/.config/agentic` |
...
The platform reads both channels' manifests hourly (`ReleaseDirectory`, `global:releases`; never GitHub's "latest release") and compares them with the machine's `hello.build.version`. `stable` follows the newest `daemon-v<semver>` tag (the first was `daemon-v0.1.1`); `latest` follows every `main` push. An automatic policy never retries a version whose last attempt failed.
...
### The folders card says locked

`agentic-daemon policy lock` was run on the machine (the policy's `locked` flag, #355): the web reads the folders but every `policy.request` is refused `policy-locked`, and the card shows the unlock command. Run `agentic-daemon policy unlock` there; the next `hello` (or the daemon's `env` frame) reconciles the folders the page wants — once, never in a loop. A card that says "The machine refused this set when it last connected" names a folder the daemon would not allow (a network share, a non-folder, one of its own): change the set, or fix the folder on the machine and reconnect the daemon (`policy show` on the machine prints what it holds).

### elevation-required

A security-sensitive machine change — the folders the web may use, browsing the machine, `bypassPermissions` on, revoke, remove — is refused `403 elevation-required: …` until the user re-confirms with the login provider (#478): the page turns it into "Confirm with GitHub to continue", keeps the change for the way back (`sessionStorage`, ten minutes) and comes back to the page for one Confirm click. A session cookie alone (a copied `__Host-session`) cannot make these changes: `/auth/elevate` sets `__Host-elevated` only when GitHub returns the same user, and only for ten minutes. History has `auth.elevated` once per such window (who, until when), then the change itself. On a preview without GitHub, the dev login's `elevate` checkbox mints the cookie.
...
5. **Paste them.** `/plugins/gmail` → turn the plugin on → **OAuth client ID** and **OAuth client secret** → Save each. They are Registry secrets (`gmail-client-id`, `gmail-client-secret`; each conduit connector names its own), sealed under `WORKSPACE_KEK` and opened only while the plugin is on and holds `secret:gmail-client-id` / `secret:gmail-client-secret`.
6. **Connect.** Click **Connect** → Google's consent screen (it warns that the app is unverified while in Testing: *Continue*) → back on `/plugins/gmail` it reads **CONNECTED** as your address. The first Connect also generates the workspace's `connector-engine-secret`: one per workspace, shared by every connector that signs in (it signs the sign-in handshake and seals the account; there is no field for it).
7. **Grant it.** An agent's config → Connectors → **gmail (conduit)** (the record the first Connect wrote). Its sessions get `gmail__search-messages`, `gmail__get-message`, `gmail__send-email` and the rest — on `anthropic-api`, and on a machine (Claude Code, Copilot CLI, Codex), where the daemon serves the tools and every call runs on the Worker over its `tool.call` (#534; a daemon that predates it serves none). Searching and reading are read-only tools; sending, replying, drafting, labelling and trashing are ruled as network (trash as destructive) actions, so an approval rule such as `network: ask` stops each one for your approval in the Inbox.
