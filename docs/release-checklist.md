...
- [ ] Tag the daemon: `git tag daemon-v<semver> && git push origin daemon-v<semver>` (or **Actions → Daemon release → Run workflow** with `tag: daemon-v<semver>`). The tag workflow attaches the five daemon zips, the harness zips (`harness-<runtime>-<version>-<os>-<arch>.zip`, three runtimes per platform — a tag release packages its own, never pointing at `daemon-latest`), their `.sha256` files and a `manifest.json` to the release `daemon-v<semver>` — nothing is attached by hand (`docs/runbook.md` §5.1).
