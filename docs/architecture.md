...
- Responsive: the published zero (0.2.0-beta.1) has no layout tier (andtii/zero-wip#473 landed upstream after it), so `@agentic/ui` carries one — `Stack`/`Row`/`Col`/`Spacer` in `packages/ui/src/layout` render `data-l-*` attributes (gap/pad on the `--space-*` ramp, align/justify/wrap/grow, breakpoint in prefix position: `data-l-md-gap`) painted by one `layout.css` step table; `useMediaQuery` is the SSR-safe `matchMedia` signal. The shell (`packages/ui/src/shell`, `AppShell`) is zero's `Navbar` + a modal `Drawer` below 768 px and a sticky sidebar above, on one hand-written `shell.css`; it is router-agnostic (the app renders `Link` through a slot). `themeInitScript` reaches `<head>` through `useHead`; `installThemes()` runs once per entry; a per-request `ThemeProvider` serves `useTheme()` on the server. The app's `build` runs `sigx zero:validate` against the consumed design system first, so CI validates it. No virtualised list in zero: the thread windows its own rows.
