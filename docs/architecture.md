...
|---|---|---|---|---|---|
| Workspace | `ws:{userId}` | save | owner, ids of agents/chats/machines/schedules, settings (tz, notification prefs, default env) | get, createAgent, createChat, registerMachinePending → {machineId, pairingCode}, listMachines, removeMachine, exportAll (task), deleteAll (task) | USR-01/02, OPS-10 |
...
| Inbox | `{ws}:inbox` | append (capped) | notifications {kind reminder/task-done/task-failed/approval/input, ref, read}, push subscriptions | list, ack, push | AST-06 |
...
Default memory: the Memory actor is the store; retrieval without embeddings — BM25-ish token scoring over text/tags/subject with kind weights (preference > lesson > fact > record > assumption) and recency decay; `conditions` matched as tags; capped block (20 entries / 4 KB). `working` entries carry a ttl; `record` entries are compacted per task. Embeddings later as a plugin via export/import (MEM-09) with a fidelity report.
...
| Web Push signer on Workers | Inbox first; push in a follow-up |
